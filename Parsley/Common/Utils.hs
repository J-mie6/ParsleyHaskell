...
module Parsley.Common.Utils (code, WQ(..), Id(..), Code, Quapplicative(..), intercalate, intercalateDiff) where
...
instance Semigroup (Id (a -> a)) where f <> g = Id $ getVal f . getVal g
instance Monoid (Id (a -> a)) where mempty = Id $ id

intercalateDiff :: (a -> a) -> [(a -> a)] -> a -> a
intercalateDiff sep xs = getVal $ intercalate (Id sep) (map Id xs)